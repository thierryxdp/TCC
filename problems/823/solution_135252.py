def faltante(pecas):
    ...
    List -> Int'''
    #Copiando a lista para nao perder a original
    pecas_copia = pecas[:]
    lista_ordem = list.sort(pecas_copia)
    quantidade_pecas = len(pecas_copia)
    Lista2 = []
    IndiceParte1 = 0
    IndiceParte2 = 0
    proximo = 0
...
#quantidade de elementos da lista inicial
    while IndiceParte1 < quantidade_pecas+1:
...
#nao for igual sera correspondente a peca faltante.
    while IndiceParte2<len(pecas_copia) and pecas_copia[IndiceParte2] == Lista2[IndiceParte2]:
