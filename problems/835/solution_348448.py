def melhor_volta(mat):
    '''funcao que retorna uma tupla informando de quem foi
    a melhor volta da prova, com qual tempo e em que volta
    list->tuple'''
    f=min(mat[0])
    g=min(mat[1])
    h=min(mat[2])
    i=min(mat[3])
    a=min(mat[4])
    b=min(mat[5])
    c=[f,g,h,i,a,b]
    d=min(c)
    if d==f:
        return 1,d,list.index(mat[0]),f)+1
    elif d==h:
        return 3,d,list.index(mat[2]),f)+1
    elif d==i:
        return 4,d,list.index(mat[3]),f)+1
    elif d==g:
        return 2,d,list.index(mat[1]),f)+1
    elif d==a:
        return 5,d,list.index(mat[4]),f)+1
    elif d==b:
        return 6,d,list.index(mat[5]),f)+1