def melhor_volta(matriz):
    '''funcao que recebe como entrada uma matriz 6x10 cujos elementos sao os tempos em segundos dos corredores em cada volta, e retorna uma tupla informando de quem foi a melhor volta, o tempo da volta e em qual volta o competidor completou o percurso com menor tempo
    list(list) -> tup(int, float, int)'''
    tempos_minimos_corredores=[]
    for linha in matriz:
        list.append(tempos_minimos_corredores,min(linha))
        tempo_minimo_geral=min(tempos_minimos_corredores)
        corredor=list.index(tempos_minimos_corredores,tempo_minimo_geral)+1
        for tempo in linha:
            if tempo==tempo_minimo_geral:
                volta=list.index(linha,tempo)+1
    return (corredor,tempo_minimo_geral_volta)