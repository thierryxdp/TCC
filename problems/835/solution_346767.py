def melhor_volta(m):
    ...
    if min(m[0]) < (min(m[1]) and min(m[2]) and min(m[3]) and min(m[4]) and min(m[5])):
        return (0, min(m[0]), list.index(m[0], min(m[0])))
    if min(m[1]) < (min(m[0]) and min(m[2]) and min(m[3]) and min(m[4]) and min(m[5])):
        return (1, min(m[1]), list.index(m[1], min(m[1])))
    if min(m[2]) < (min(m[0]) and min(m[1]) and min(m[3]) and min(m[4]) and min(m[5])):
        return (2, min(m[2]), list.index(m[2], min(m[2])))
    ...
    if min(m[5]) < (min(m[0]) and min(m[1]) and min(m[2]) and min(m[3]) and min(m[4])):
        return (5, min(m[5]), list.index(m[5], min(m[5])))
    else:
        return None