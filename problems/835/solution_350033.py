def melhor_volta(matriz):
    ...
    menores_tempos=[]
    for i in matriz:
        menores_tempos= menores_tempos + [min(i)]
    menor_tempo_total= min(menores_tempos)
    corredor=list.index(menores_tempos,menor_tempo_total)
    volta= list.index(matriz[corredor],menor_tempo_total)
    return (corredor+1,menor_tempo_total,volta+1)