def melhor_volta(matriz):
    '''Dada uma matriz de entrada referente a pista de kart a função calcula e retorna uma tupla contendo as informações de quem foi a melhor volta, em qual tempo foi e qual foi a volta.
    list-> tuple'''
    listaminimos = []
    for m in range(len(matriz)):
        listaminimos = listaminimos + [min(matriz[m])]
    qualtempo = min(listaminimos)
    quemfoi = list.index(listaminimos,qualtempo)
    qualvolta = list.index(matriz[quemfoi],qualtempo)
    return (quemfoi + 1,qualtempo,qualvolta + 1)