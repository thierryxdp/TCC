# Função que dada uma lista de numeros inteiros e um número inteiro n, retorna outra lista, que contenha todos os números
# da lista original maiores que n, ordenados em ordem crescente
def maiores(lista_int,n):
    if lista_int[-1] < n:
     del lista_int[-1]
    if len(lista_int) >= 1 and lista_int[-1] < n:
     del lista_int[-1]
    if len(lista_int) >= 1 and lista_int[-1] < n:
     del lista_int[-1]
    if len(lista_int) >= 1 and lista_int[-1] < n:
     del lista_int[-1]
    if len(lista_int) >=2 and lista_int[-2] < n:
     del lista_int[-2]
    if len(lista_int) >=2 and lista_int[-2] < n:
     del lista_int[-2]
    if len(lista_int) >=2 and lista_int[-2] < n:
