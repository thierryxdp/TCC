...
def inverte(frase):
    ...
    lista_virar=str.split(minusculo)
    invertido=list.reverse(lista_virar)
    volta_str=str.join(',',lista_virar)
    tirar_virgula=str.replace(volta_str,',',' ',(str.count(volta_str),',',)))
    final_invert=tirar_virgula
