...
def inverte(frase):
    '''função que recebe uma frase(string) e retorna uma outra frase, sem a
pontuação, sem letras maiúsculas, e com a ordem inversa da frase de entrada;
string->string'''

    frase2=str.lower(frase)
    frase3= retira_pontuacao(frase2)
    frase4= str.strip(frase3)
    lista= str.split(frase4)
    list.reverse(lista)
