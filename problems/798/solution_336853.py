def freq_palavras(frases):
    ...
    for elemento in lista:
        if not elemento in listanova:
            ...
            list.append(listanova,list.count(lista,elemento))
    for elemento in listanova:
        if list.index(listanova,elemento)%2 ==0:
            dicionario[listanova[list.index(listanova,elemento)]]=elemento[list.index(listanova,elemento)+1]
    return dicionario