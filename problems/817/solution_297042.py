...
def acima_da_media(lista):
    ...
    elementosiniciais=len(lista)
    media=sum(lista)/len(lista)
    list.insert(lista,0,media)
    list.sort(lista)
    indice=list.index(lista,media)+1
    elementosfinais=len(lista)
    repetiu=list.count(lista,media)
    
    if elementosiniciais<2:
        return []
    elif repetiu>1:
        return lista[indice+1:elementosfinais]
    else:
    	return lista[indice:elementosfinais]