...
def acima_da_media(notas_alunos):
    ...
    ordem = sorted(notas_alunos + [media])

    x = ordem.index(media)

    novaLista = ordem[x:]

    novaLista_final = novaLista[1:]

    if len(notas_alunos)==1:

        novaLista.clear()

        return novaLista[:]
    
    if novaLista[0] == media:


        novaLista_final2=novaLista.remove(media)

    
        return novaLista_final2
    
    

    return novaLista_final