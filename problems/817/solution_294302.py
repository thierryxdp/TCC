def acima_da_media(lista:list)->list:
    ...
    media = int(soma_das_notas / quantidade_numeros)
    list.append(lista,media)
    list.sort(lista)
    copia_da_lista = list.copy(lista)
    indice_media = list.index(lista,media)
    if media in copia_da_lista:
        return lista[indice_media+2:]
    else:
        return lista[indice_media:]